class Chart
  ...
  # 各グラフを表示するためのメソッド
  def aggregate_rate
    aggregate_loan_rate(@bank, :rate)
  end

  def aggregate_age
    aggregate_loan_age(@bank, :age)
  end

  def aggregate_income
    aggregate_loan_income(@bank, :income)
  end

  def aggregate_family_form
    aggregate_loan_family_form(@bank)
  end

  def aggregate_job_period
    aggregate_loan_job_period(@bank)
  end
